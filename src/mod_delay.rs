...
impl ModulatedDelay {
    pub fn new(delay_ms: f32, mod_freq: f32, sample_rate_hz: f32) -> Self {
        ModulatedDelay {
            delay_line: DelayLine::new((MAX_DELAY_MS * sample_rate_hz / 1000.0) as usize), // 100 ms delay
            lfo: Lfo::new(WaveformType::Triangle, mod_freq, sample_rate_hz),
            lfo_depth: 1.0,
...
mod tests {
    ...
    fn test_triangular_modulation() {
        ...
        let mod_samples = 2 * (0.1 * delay_samples as f32) as usize;

        let mut mod_delay = ModulatedDelay::new(delay_ms, mod_freq, sample_rate);
